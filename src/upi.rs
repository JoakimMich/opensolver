...
struct TreeInformation {
    ...
    board: Option<String>,
    /// Isomorphism for trees starting on the flop / on the turn
...
pub struct CliSession {
    ...
    pub hand_order_map: HashMap<String, usize>,
    ...
    pub fn run_command(&mut self, user_input: &str) -> bool {
        if user_input.len() != 0 && user_input.chars().nth(0).unwrap() != '#' {
            ...
            match input_params[0] {
                ...
                "set_isomorphism" => set_isomorphism(&input_params, &mut self.tree_information),
...
fn set_accuracy(input_params: &Vec<&str>, accuracy: &mut Accuracy) {
    ...
    if input_params.len() == 1 {
        ...
        let accuracy_type = if input_params.len() > 2 {
...
// set_isomorphism <flop trees> <turn trees>: 1 skips isomorphic cards in trees starting on the
// flop / turn (default 1 0, as in PioSolver)
fn set_isomorphism(input_params: &Vec<&str>, tree_information: &mut TreeInformation) {
    if input_params.len() < 3 || !["0", "1"].contains(&input_params[1]) || !["0", "1"].contains(&input_params[2]) {
        println!("ERROR: {} incorrect or missing argument", input_params[0]);
    } else {
        tree_information.isomorphism = (input_params[1] == "1", input_params[2] == "1");
        println!("{} ok!", input_params[0]);
    }
}
    ...
    match trainer_option {
        Some(_trainer) => {
            if input_params.len() < 3 {
                ...
                let _oop = if input_params[1] == "oop" || input_params[1] == "OOP" {
                    true
                } else if input_params[1] == "ip" || input_params[1] == "IP" {
                    false
                } else {
...
fn build_tree(tree_information: &mut TreeInformation, trainer: &mut Option<Trainer>) {
    if tree_information.eff_stack.is_none() || tree_information.pot.is_none() || tree_information.oop_range.is_none() || tree_information.ip_range.is_none() || tree_information.lines.is_none() || tree_information.board.is_none() {
        ...
        range_manager.isomorphism = if tree_information.board.as_ref().unwrap().len() == 6 {
            tree_information.isomorphism.0
        } else {
            tree_information.isomorphism.1
        };
        let new_trainer = Trainer::new(range_manager, tree_information.lines.as_ref().unwrap().clone(), tree_information.eff_stack.unwrap(), tree_information.pot.unwrap());
