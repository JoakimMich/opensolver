...
/// 1: index of card 2
/// 2: weight of combo
/// 3: user-defined ranking 
#[derive(Debug, Clone, Copy)]
pub struct Combo(pub u8, pub u8, pub u8, pub u16, pub Option<u16>);
...
impl Combo {
    ...
    pub fn update_joint(&mut self, range: &HandRange) {
        match range.hands.iter().position(|&r| r == *self) {
            Some(x) => { 
                self.4 = Some(x as u16);
            },
            None => ()
        };
    }
...
impl HandRange {
    ...
    fn parse_hand(&mut self, i: &mut usize) -> bool {
        ...
        let explicit_suits = self.parse_suit(i, &mut s1);
    ...
    fn add_combo(&mut self, c1: u8, c2: u8, weight: u8) {
        ...
        let h = if c1 >> 2 < c2 >> 2 || (c1 >> 2 == c2 >> 2 && (c1 & 3) < (c2 & 3)) {
            Combo(c2, c1, weight, 0, None)
        } else {
            Combo(c1, c2, weight, 0, None)
        };
