...
fn test_iso() {
    ...
    let mut oop_range = HandRange::from_string("AKs".to_string());
    let ip_range = HandRange::from_string("QQ+,AKs".to_string());
    let tree_board = "QcJc2s".to_string();
    oop_range.remove_isomorphic(&tree_board);
    for h in &oop_range.hands {
        println!("{}",h.to_string());
    }

    //isomorphism_mapping(&"AhKdQh2d".to_string());
...
fn test_river() {
    ...
    let tree_board = "QsJh2h8d2d".to_string();
